...
function renderItems(items) {
  ...
  if (itemsHint) itemsHint.textContent = `${lastItems.length} 件（チェックした行だけ送れます）`;
...
async function onExtract() {
  ...
  setStatus("LLM 抽出中（数十秒かかることがあります）");
  try {
...
async function onCommit() {
  ...
  try {
    ...
    setStatus(`保存 ${data.saved ?? 0} 件 → proposals キューへ`);
    ...
    commitBtn.textContent = "選択分をキューへ";
  }
}
