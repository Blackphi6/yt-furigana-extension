...
3. **バンドル人名フレーズ** — japanese-personal-name-dataset（上位姓）+ NEologd 抽出。
...
6. **地名** — `npm run dict:place-names`（Geolonia / ABR / 地名集日本 / GeoNLP / KEN_ALL ギャップ）。詳細は [`PLACE-NAME-DATASETS.md`](PLACE-NAME-DATASETS.md)。
