...
} from "./context-reading-overlay.js?v=20260824b";
...
/**
 * After a click/table fix, optionally stage for LLM review.
 * @param {string} surface
 * @param {string} reading
 */
async function maybeSubmitClickProposal(surface, reading) {
  const share = $("#pin-share-proposal")?.checked !== false;
  if (!share) {
    showProposeStatus(
      "この文には反映しました（共有候補への送信はオフ）。",
      { ok: true }
    );
    return;
  }
  try {
    const prop = await submitProposals(
      [{ surface, reading }],
      { source: "demo-click" }
    );
    const sum = prop.summary || {};
    const llmNote = prop.llmQueued
      ? "LLM審査をキューに入れました。"
      : prop.llm
        ? "審査中です。"
        : "ヒューリスティック通過・審査待ち（LLMキー未設定時は管理者処理待ち）。";
    showProposeStatus(
      `共有候補へ送信（即公開なし）。待ち ${sum.pending || 0} / 承認 ${sum.accepted || 0} / 却下 ${sum.rejected || 0}。${llmNote}`,
      { ok: true }
    );
  } catch (err) {
...
async function runAnalyze(options = {}) {
  ...
  try {
    ...
    if (share && pins.length) {
      ...
      // status is set by maybeSubmitClickProposal
    }
  } catch (err) {
    const msg = String(err.message || err);
